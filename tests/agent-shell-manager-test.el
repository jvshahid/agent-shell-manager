...
(ert-deftest agent-shell-manager-test/unseen-idle-set-on-busy-to-idle-hidden ()
  (agent-shell-manager-test--reset-state)
  (let ((s1 (agent-shell-manager-test--make-mock-session "*a*" "/tmp/" nil)))
    (agent-shell-manager-test--with-mock-sessions (list s1)
      (puthash s1 t agent-shell-manager--last-busy)
      (cl-letf (((symbol-function 'agent-shell-manager--buffer-visible-p)
                 (lambda (_) nil)))
        (agent-shell-manager--update-unseen-state))
      (should (gethash s1 agent-shell-manager--unseen-idle)))))

(ert-deftest agent-shell-manager-test/unseen-idle-not-set-when-visible ()
  (agent-shell-manager-test--reset-state)
  (let ((s1 (agent-shell-manager-test--make-mock-session "*a*" "/tmp/" nil)))
    (agent-shell-manager-test--with-mock-sessions (list s1)
      (puthash s1 t agent-shell-manager--last-busy)
...
(ert-deftest agent-shell-manager-test/unseen-idle-cleared-when-buffer-visible ()
  (agent-shell-manager-test--reset-state)
  (let ((s1 (agent-shell-manager-test--make-mock-session "*a*" "/tmp/" nil)))
    (agent-shell-manager-test--with-mock-sessions (list s1)
